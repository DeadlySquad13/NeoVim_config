-- Reference: https://github.com/folke/snacks.nvim/blob/fe7cfe9800a182274d0f868a74b7263b8c0c020b/lua/snacks/rename.lua
local rename = {}
...
function rename.rename_file(opts)
  opts = opts or {}
  ...
  local function rename()
    ...
    rename.on_rename_file(from, to, function()
      local ok = rename._rename(from, to)
...
function rename._rename(from, to)
  ...
  if ret ~= 0 then
    notify("Failed to rename file: `" .. from .. "`", vim.log.levels.ERROR)
    return false
  end
...
---@param to string
---@param rename? fun()
function rename.on_rename_file(from, to, rename)
  local changes = { files = { {
