...
---@param use_groups boolean
---@return LazyKeysSpec
local function wk_to_lazy(wk, wk_opts, wk_groups, use_groups)
  if use_groups == nil then
    use_groups = true
  end
  ...
  -- We don't use 'label'.
  -- legendary.desc = wk.label or vim.tbl_get(wk, 'opts', 'desc')

  return vim.tbl_extend('error', legendary, wk.opts or {})
end
...
function M.parse_whichkey(which_key_tbls, which_key_opts, do_binding, use_groups)
  ...
  local wk_parsed = require('ds_omega.ds_omega_utils.which_key').parse(which_key_tbls, which_key_opts)
  ...
  vim.tbl_map(function(wk)
    if vim.tbl_get(wk, 'opts', 'desc') and wk.group ~= true then
      table.insert(legendary_tbls, wk_to_lazy(wk, which_key_opts, wk_groups, use_groups))
    end
  end, wk_parsed)
